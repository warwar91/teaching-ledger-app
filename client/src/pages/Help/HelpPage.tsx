...
const sections: HelpSection[] = [
  {
    ...
    items: [
      {
        q: '如何注册账号？',
        a: '在登录页面点击"注册账号"，输入用户名和密码即可注册。用户名和密码长度均需至少 6 位。',
      },
      {
        q: '如何成为管理员？',
      ...
      {
        q: '如何修改密码？',
        a: '登录后，点击右上角用户头像/用户名，在下拉菜单中选择"修改密码"。需要先输入原密码，再输入两次新密码确认。新密码要求8~128位，必须同时包含大小写字母和数字。修改成功后所有设备的登录状态会失效，需重新登录。',
      },
      {
        q: '忘记密码怎么办？',
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        a: '点击公告进入详情页，勾选需要的任务条目（可多选），点击"添加到我的台账"按钮，在弹窗中选择目标台账文件夹，可重新设置预计完成时间、主要执行人和备注，最后点击"确认添加"即可。添加后任务会自动进入您选择的台账，状态默认为"未处理"。',
      },
      {
        q: '公告中的附件如何查看？',
